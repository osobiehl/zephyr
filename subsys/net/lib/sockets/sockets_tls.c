				      ...
				      struct zsock_pollfd *pfd, struct k_poll_event **pev)
{
	...
	*pev = socket_events;
	...
	if (socket_events == *pev) {
		/* The unchanged cursor means the backend prepared no socket
		 * events with POLLIN masked. Reuse the handshake slot when
		 * preparing with POLLIN enabled.
		 */
		socket_events = handshake;
	} else {
		/* The cursor advanced over socket events prepared by the backend.
		 * Keep their positions and disable the completed semaphore so
		 * following descriptors stay aligned.
		 */
